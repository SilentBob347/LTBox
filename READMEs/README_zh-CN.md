...
**`11. 对固件应用增量 OTA 更新`**
将增量 OTA 更新包应用到固件文件。处理有效载荷修补、AVB 重新签名（可选）、super 分区重建和 XML 更新。

**`12. 解包 super 分区`**
从分割的 `super_*.img` 文件中提取动态分区镜像。

**`13. 重新打包 super 分区`**
从动态分区镜像重建分割的 `super_*.img` 输出文件，并更新 rawprogram XML。

**`14. 使用测试密钥重新签名固件`**
使用 AOSP 测试密钥重新签名 AVB 镜像，并重建 `vbmeta.img` / `vbmeta_system.img`。
