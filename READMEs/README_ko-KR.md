...
**`11. 펌웨어에 증분 OTA 적용`**
증분 OTA 업데이트 패키지를 펌웨어 파일에 적용합니다. 페이로드 패치, AVB 재서명(선택), super 파티션 재빌드, XML 업데이트를 처리합니다.

**`12. super 파티션 언팩`**
분할된 `super_*.img` 파일에서 다이나믹 파티션 이미지를 추출합니다.

**`13. super 파티션 리팩`**
다이나믹 파티션 이미지로부터 분할된 `super_*.img` 출력 파일을 재빌드하고 rawprogram XML을 업데이트합니다.

**`14. 테스트 키로 펌웨어 재서명`**
AOSP 테스트 키로 AVB 이미지를 재서명하고 `vbmeta.img` / `vbmeta_system.img`를 재빌드합니다.
