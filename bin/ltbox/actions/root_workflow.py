...
def _patch_root_image_from_image_folder(
    ...
    root_ui.echo(get_string("act_wait_image").format(image=wait_image))
    ...
    for fname in strategy.required_files:
        ...
        try:
            shutil.copy(src, dst)
            root_ui.echo(get_string("act_copy_boot").format(name=src.name))
        except (IOError, OSError) as e:
            root_ui.error(get_string("act_err_copy_boot").format(name=src.name, e=e))
    ...
    root_ui.echo(get_string("act_backup_boot"))
    shutil.copy(
    ...
    with utils.temporary_workspace(const.WORK_DIR):
        ...
        if isinstance(strategy, LkmRootStrategy) and not lkm_kernel_version:
            root_ui.echo(get_string("err_req_kernel_ver_lkm"))
            lkm_kernel_version = root_ui.read_input(
                get_string("prompt_enter_kernel_version")
            ).strip()
            if not lkm_kernel_version:
                root_ui.error(get_string("err_kernel_version_req"))
                return False
    ...
    if patched_boot_path and patched_boot_path.exists():
        root_ui.echo(get_string("act_finalize_root"))
        ...
        width = root_ui.get_term_width()
        root_ui.echo("  " + "=" * width)
        root_ui.echo(get_string("act_success"))
        ...
        root_ui.error(get_string("act_err_root_fail_image").format(image=fail_image))
        return False
...
def patch_root_image_file(gki: bool = False, root_type: str = "ksu") -> None:
    ...
    root_ui.echo(get_string("act_clean_dir").format(dir=strategy.log_output_dir_name))
...
def patch_root_image_file_and_flash(
    ...
    confirm = (
        root_ui.prompt(get_string("prompt_flash_image_folder_confirm")).strip().lower()
    )
    if confirm != "y":
        return
    ...
    if active_slot:
        root_ui.echo(get_string("act_active_slot").format(slot=active_slot))
    else:
        root_ui.echo(get_string("act_warn_root_slot"))
        if gki:
...
def _prepare_root_env(strategy: RootStrategy):
    root_ui.echo(get_string("act_start_root"))
    ...
    if strategy.requires_kernel_version:
        if not dev.skip_adb:
            try:
                ...
                root_ui.error(get_string("act_root_warn_lkm_kver_fail").format(e=e))
                root_ui.error(get_string("act_root_warn_lkm_kver_retry"))
        else:
            root_ui.error(get_string("act_root_err_lkm_skip_adb"))
...
def _dump_and_generate_root_image(
    ...
    with utils.temporary_workspace(const.WORKING_BOOT_DIR):
        ...
        with dev.edl_session(auto_reset=True, reset_msg_key="act_dump_reset") as port:
            ...
            root_ui.echo(get_string("act_temp_backup_avb"))
            ...
            root_ui.echo(get_string("act_backups_done"))
        ...
        try:
            ...
            root_ui.echo(get_string("act_root_step5"))
            ...
            if isinstance(e, ToolError):
                ...
                root_ui.error(get_string("act_err_avb_footer").format(e=e))
...
def _flash_root_image(
    ...
    if not dev.skip_adb:
        root_ui.echo(get_string("act_wait_sys_adb"))
        dev.adb.wait_for_device()
        root_ui.echo(get_string("act_reboot_edl_flash"))
    else:
        root_ui.echo(get_string("act_skip_adb_on"))
        root_ui.echo(get_string("act_manual_edl_now"))
    ...
    with dev.edl_session(auto_reset=True, reset_msg_key="act_reset_sys") as port:
        try:
            ...
            root_ui.error(get_string("act_err_edl_write").format(e=e))
            raise
...
def root_device(
    ...
    root_ui.echo(get_string("act_root_step1"))
    if not dev.skip_adb:
        dev.adb.wait_for_device()
    ...
    if not strategy.download_resources(lkm_kernel_version):
        root_ui.error(get_string("err_download_resources_abort"))
        return
    ...
    root_ui.echo(get_string("act_root_step2"))
    ...
    width = root_ui.get_term_width()
    root_ui.echo("\n" + "!" * width)
    root_ui.error(get_string("act_root_warn_brick"))
    root_ui.echo("!" * width + "\n")
    root_ui.echo(get_string("act_root_finish"))


def unroot_device(dev: device.DeviceController) -> None:
    root_ui.echo(get_string("act_start_unroot"))
    ...
    if len(available_strategies) > 1:
        ...
        if choice == "m":
            root_ui.echo(get_string("act_op_cancel"))
            return
        ...
        root_ui.echo(get_string(selected_strategy.unroot_detect_msg_key))
    else:
    ...
    root_ui.echo(get_string("act_unroot_step1"))
    edl.ensure_edl_requirements()
    root_ui.echo(get_string("act_unroot_step3"))
    ...
    root_ui.echo(get_string("act_unroot_finish"))


def sign_and_flash_twrp(dev: device.DeviceController) -> None:
    root_ui.echo(get_string("act_start_rec_flash"))
    ...
    root_ui.echo(get_string("act_wait_image"))
    ...
    with utils.temporary_workspace(const.WORK_DIR):
        ...
        with dev.edl_session(auto_reset=True, reset_msg_key="act_dump_reset") as port:
            root_ui.echo(get_string("act_dump_recovery").format(part=target_partition))
            try:
            ...
            root_ui.echo(get_string("act_backup_recovery_ok"))
        ...
        if not key_file:
            root_ui.error(get_string("img_err_boot_key_mismatch").format(key=pubkey))
...
def _install_manager_apk(dev: device.DeviceController):
    ...
    width = root_ui.get_term_width()
    root_ui.echo("\n" + "-" * width)
    root_ui.echo(get_string("act_install_ksu").format(name="Manager App"))

    if not manager_apk.exists():
        root_ui.error(get_string("act_manager_apk_not_found"))
        return

    if dev.skip_adb:
        root_ui.echo(get_string("act_adb_skipped_manual_install"))
        root_ui.echo(get_string("act_file_location").format(path=manager_apk))
        return
    ...
    try:
        ...
        root_ui.echo(get_string("act_ksu_ok"))
    except Exception as e:
        root_ui.error(get_string("act_err_ksu").format(e=e))
    root_ui.echo("-" * width + "\n")
