...
def ensure_payload_dumper() -> None:
    import subprocess

    if const.PAYLOAD_DUMPER_PY.exists():
        return

    utils.ui.echo(get_string("dl_payload_dumper_not_found"))
    const.DOWNLOAD_DIR.mkdir(parents=True, exist_ok=True)

    repo_zip_url = (
        "https://github.com/miner7222/payload_dumper/archive/refs/heads/master.zip"
    )
    temp_zip = const.DOWNLOAD_DIR / "payload_dumper.zip"

    try:
        download_resource(repo_zip_url, temp_zip, timeout=30, retries=1, backoff=2)

        utils.ui.echo(get_string("dl_payload_dumper_extracting"))
        with zipfile.ZipFile(temp_zip, "r") as zf:
            zf.extractall(const.DOWNLOAD_DIR)

        # GitHub archives extract to <repo>-<branch>/ (e.g. payload_dumper-master/)
        extracted_dir = const.DOWNLOAD_DIR / "payload_dumper-master"
        if not extracted_dir.exists():
            # Fallback: find the extracted directory
            candidates = [
                d
                for d in const.DOWNLOAD_DIR.iterdir()
                if d.is_dir() and d.name.startswith("payload_dumper")
            ]
            if not candidates:
                raise ToolError(
                    get_string("dl_payload_dumper_failed").format(
                        e="extracted directory not found"
                    )
                )
            extracted_dir = candidates[0]

        if const.PAYLOAD_DUMPER_DIR.exists():
            shutil.rmtree(const.PAYLOAD_DUMPER_DIR)
        extracted_dir.rename(const.PAYLOAD_DUMPER_DIR)

        # Install requirements
        req_file = const.PAYLOAD_DUMPER_DIR / "requirements.txt"
        if req_file.exists():
            utils.ui.echo(get_string("dl_payload_dumper_installing_deps"))
            subprocess.run(
                [str(const.PYTHON_EXE), "-m", "pip", "install", "-r", str(req_file)],
                check=True,
                capture_output=True,
                text=True,
            )

    except (ToolError, OSError, zipfile.BadZipFile, subprocess.CalledProcessError) as e:
        if const.PAYLOAD_DUMPER_DIR.exists():
            shutil.rmtree(const.PAYLOAD_DUMPER_DIR)
        raise ToolError(get_string("dl_payload_dumper_failed").format(e=e)) from e
    finally:
        if temp_zip.exists():
            temp_zip.unlink()

    if not const.PAYLOAD_DUMPER_PY.exists():
        raise ToolError(
            get_string("dl_payload_dumper_failed").format(
                e="payload_dumper.py not found after extraction"
            )
        )

    utils.ui.echo(get_string("dl_payload_dumper_ready"))
